...
class RemoveRole extends Command
{
     ...
     * @var string
     */
    protected $signature = 'lapermission:remove-role
                                    {user_id? : User ID}
                                    {role_id? : Role ID}';
    ...
    public function handle()
    {
        ...
        {
            LaPermission::removeRoleFromUser($user->id, $role->id);
            $this->info("Successfully removed role $role->name to $user->name");
        }
        else
        {
            $this->error("User does not have '$role->name' role.");
        }
    }
}
